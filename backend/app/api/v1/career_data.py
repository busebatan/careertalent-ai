...
router = APIRouter()
require_career_data_admin = require_admin_permission("career_data.manage")


@router.get("/roles", response_model=list[CareerDataRoleResponse])
def roles(db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> list[dict]:
...
def create_role(payload: CareerDataRoleWrite, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> dict:
    return career_catalog_service.create_role(db, payload)


@router.put("/roles/{role_id}", response_model=CareerDataRoleResponse)
def update_role(role_id: int, payload: CareerDataRoleWrite, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> dict:
    return career_catalog_service.update_role(db, role_id, payload)


@router.delete("/roles/{role_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_role(role_id: int, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> Response:
    career_catalog_service.delete_role(db, role_id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)


@router.get("/skills", response_model=list[CareerSkillResponse])
def skills(db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> list[dict]:
...
def create_skill(payload: CareerSkillWrite, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> dict:
    return career_catalog_service.create_skill(db, payload)


@router.put("/skills/{skill_id}", response_model=CareerSkillResponse)
def update_skill(skill_id: int, payload: CareerSkillWrite, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> dict:
    return career_catalog_service.update_skill(db, skill_id, payload)


@router.delete("/skills/{skill_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_skill(skill_id: int, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> Response:
    career_catalog_service.delete_skill(db, skill_id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)


@router.get("/sources", response_model=list[CareerDataSourceResponse])
def sources(db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> list[dict]:
...
def create_source(payload: CareerDataSourceWrite, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> dict:
...
def update_source(source_id: int, payload: CareerDataSourceWrite, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> dict:
    return career_catalog_service.update_source(db, source_id, payload)


@router.delete("/sources/{source_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_source(source_id: int, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> Response:
...
def requirements(db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> list[dict]:
...
def create_requirement(payload: CareerRoleSkillRequirementWrite, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> dict:
...
def update_requirement(requirement_id: int, payload: CareerRoleSkillRequirementWrite, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> dict:
...
def delete_requirement(requirement_id: int, db: Session = Depends(get_db), _admin: User = Depends(require_career_data_admin)) -> Response:
