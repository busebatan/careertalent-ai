...
def _page(org,position,source=None):
    ...
    return PublicPositionPageResponse(organization=PublicOrganizationResponse(name=org.name,slug=org.slug,website=org.website,logo_url=settings.get("logo_url")),position=PublicPositionResponse(
        ...
        estimated_application_minutes=int(evaluation.get("estimated_application_minutes",8)),estimated_assessment_minutes=int(evaluation["estimated_assessment_minutes"]) if evaluation.get("estimated_assessment_minutes") is not None else None),source=source)
...
def page(organization_slug:str,position_path:str,db:DB):
    ...
    _visibility(position); return _page(org,position)
...
def short(short_code:str,db:DB):
    ...
    return _page(org,position,{"short_code":link.short_code,"channel":link.channel,"label":link.label,"campaign":link.campaign})
...
def submit(public_id:str,payload:CandidatePositionApplicationCreate,response:Response,db:DB,candidate:Candidate):
    ...
    app=RecruitingApplication(id=str(uuid4()),organization_id=org.id,position_id=position.id,candidate_user_id=candidate.id,candidate_name=candidate.full_name,candidate_email=candidate.email,
        ...
        application_snapshot={"cv":cv_snapshot,"selected_projects":payload.selected_projects,"criteria_version":{"id":criteria.id,"version":criteria.version_number,"criteria":criteria.criteria} if criteria else None},
