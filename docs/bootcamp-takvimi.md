# YZTA Bootcamp 2026 — Takvim ve Raporlar

> **Proje:** CareerTalent AI — Grup 92  
> **Mimari:** Plan A (FastAPI + Laravel) ile başlanır; Sprint 1 veya 2 sonrası Plan B değerlendirilebilir.  
> **Son güncelleme:** 2026-07-05

---

## Genel takvim

| Faz | Tarih aralığı | Rapor dosyası | Durum |
|-----|---------------|---------------|-------|
| Takımların açıklanması | 12 Haziran 2026 | [sprint-0-takim-kurulumu.md](sprintler/sprint-0-takim-kurulumu.md) | Tamamlandı |
| **1. Sprint** | 19 Haziran – 5 Temmuz 2026 | [sprint-1-ilk-sprint.md](sprintler/sprint-1-ilk-sprint.md) | Tamamlandı |
| **2. Sprint** | 6 Temmuz – 19 Temmuz 2026 | [sprint-2-ikinci-sprint.md](sprintler/sprint-2-ikinci-sprint.md) | **Aktif** |
| Soru-cevap dönemi | 6 Temmuz 20:00 – 20 Temmuz 20:00 | [soru-cevap-donemi.md](sprintler/soru-cevap-donemi.md) | Planlandı |
| **Son Sprint** | 20 Temmuz – 2 Ağustos 2026 | [sprint-3-son-sprint.md](sprintler/sprint-3-son-sprint.md) | Planlandı |
| Değerlendirme süreci | 3 Ağustos – 13 Ağustos 2026 | [degerlendirme-sureci.md](sprintler/degerlendirme-sureci.md) | Planlandı |
| Top 10 sunum | 14 Ağustos 2026 | [top10-sunum.md](sprintler/top10-sunum.md) | Planlandı |

```
12 Haz    19 Haz──────5 Tem    6 Tem──────19 Tem    20 Tem──────2 Ağu    3-13 Ağu    14 Ağu
  │         │  Sprint 1  │       │ Sprint 2 │         │ Son Sprint │      Değerlendirme  Top10
 Takım      └────────────┘       └──────────┘         └────────────┘
 açıklandı         ↑                      ↑
              Plan B? retro           Plan B? retro
              (Sprint 1 sonu)         (Sprint 2 sonu)
```

---

## Rapor disiplini

Her faz dosyasında üç bölüm vardır:

1. **Plan** — sprint başında ne yapılacak (sabit)
2. **Günlük notlar** — sprint içinde doldurulur
3. **Sprint / faz sonu raporu** — teslim günü doldurulur, jüriye hazır özet

Sprint sonu raporu şablonu: [sprint-rapor-sablonu.md](sprintler/sprint-rapor-sablonu.md)

---

## Takım

| İsim | Rol |
|------|-----|
| Buse Batan | Scrum Master + Frontend + Teknik Mimari |
| Bithanya Abraham Haile | Frontend + Sunum |
| Döne Sakız | Backend (FastAPI) |
| Yiğit Dede | Veri & Analiz + Product Owner  |

---

## İlgili dokümanlar

- [Teknik Mimari](teknik-mimari.md)
- [Bootcamp Takvimi](bootcamp-takvimi.md)
- [Ürün Değeri ve Fark (sürümlü)](urun/README.md)
