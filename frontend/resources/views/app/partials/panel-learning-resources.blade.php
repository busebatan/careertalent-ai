@php
    $isPreview = ($mode ?? 'full') === 'preview';
    $visibleResources = array_map(static fn (array $resource): array => [
...
>
    ...
    <div class="space-y-3">
        @if ($isPreview)
            @foreach (array_slice($visibleResources, 0, 3) as $item)
                <article class="panel-entry flex flex-col gap-2 p-3">
                    ...
                    <p class="text-xs text-slate-500">{{ $item['provider'] }}</p>
                </article>
            @endforeach
        @else
            <template x-for="item in filtered" :key="item.id">
                <article class="panel-card flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                        ...
                        <p class="text-sm text-slate-500" x-text="item.provider"></p>
