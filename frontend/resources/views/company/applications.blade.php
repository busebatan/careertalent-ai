            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-[760px] text-left text-sm">
                    <thead class="border-b border-slate-200 bg-slate-50 text-xs uppercase text-slate-500 dark:border-slate-800 dark:bg-slate-900/60"><tr>
                        <th class="px-5 py-4">{{ __('company.applications.candidate') }}</th>
                        <th class="px-5 py-4">{{ __('company.applications.position') }}</th>
                        <th class="px-5 py-4">{{ __('company.applications.stage') }}</th>
                        <th class="px-5 py-4">{{ __('company.applications.applied_at') }}</th>
                        <th class="px-5 py-4">{{ __('company.applications.retention') }}</th>
                    </tr></thead>
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                        @foreach($applications as $application)
                            <tr x-show="isVisible(@js($application))" x-cloak>
                                ...
                                <td class="px-5 py-4">@include('company.partials.stage-badge', ['stage' => $application['current_stage']])</td>
                        ...
                        <tr x-show="visibleCount() === 0" x-cloak>
                            <td class="px-5 py-8 text-center text-sm text-slate-500" colspan="5">{{ __('company.applications.no_results') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
