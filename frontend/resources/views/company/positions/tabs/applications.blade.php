...
<section class="panel-card overflow-hidden">
    ...
    @else
        <div class="overflow-x-auto"><table class="min-w-[1100px] w-full text-left text-sm"><thead class="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-900"><tr><th class="px-5 py-4">{{ __('company.applications.candidate') }}</th><th class="px-4 py-4">{{ __('company.applications.stage') }}</th><th class="px-4 py-4">Tamamlanma durumu</th><th class="px-4 py-4">Eksik belgeler</th><th class="px-5 py-4">Son işlem tarihi</th>@if($canManageApplications)<th class="px-5 py-4">İnsan incelemesi</th>@endif</tr></thead><tbody class="divide-y divide-slate-200 dark:divide-slate-800">
            @foreach($applications as $application)
                <tr class="align-top">
                    <td class="px-5 py-4"><strong>{{ $application['candidate_name'] }}</strong><span class="panel-muted block text-xs">{{ $application['candidate_email'] ?? '' }}</span></td>
                    <td class="px-4 py-4">{{ __('company.applications.stage_'.($application['stage'] ?? 'new')) }}</td>
                    <td class="px-4 py-4">{{ $application['completion_status'] ?? '—' }}</td>
                    <td class="px-4 py-4">{{ implode(', ', $application['missing_documents'] ?? []) ?: '—' }}</td>
